...
1. **Query Slack:** Run the Python script to get all project channels
   ...
   - Execute: `python ~/_apps/.claude/scripts/list_all_proj_channels_slack.py`
   ...
   - The script outputs into a dirctory "~/_apps/.claude/tmp_output/"
   - Parse the JSON output to get the channel list

2. **Read existing data:** Read `~/_apps/.claude/command_resources/projects_to_monitor.md` to see what projects are already tracked

3. **Update the file:** For EACH project channel found in Slack:
   - Check if the channel already exists in `~/_apps/.claude/projects_to_monitor.md` (by channel ID or name)
...
✅ All project channels have been saved to `~/_apps/.claude/command_resources/projects_to_monitor.md`
